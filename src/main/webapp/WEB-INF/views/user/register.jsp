        ...
        <c:if test="${registerResult != null}">
            <c:choose>
                <c:when test="${registerResult == RegisterResult.SUCCESS}">
                    <c:set var="alert" value="가입하신 이메일로 인증번호를 보냈으니 1시간 내에 확인해주세요." />
                </c:when>
                <c:when test="${registerResult == RegisterResult.NORMALIZATION_FAILURE}">
                    <c:set var="alert" value="유효성 검사에 실패하였습니다." />
                </c:when>
                <c:when test="${registerResult == RegisterResult.DUPLICATE_EMAIL}">
                    <c:set var="alert" value="중복된 이메일입니다." />
                </c:when>
                <c:when test="${registerResult == RegisterResult.DUPLICATE_NICKNAME}">
                    <c:set var="alert" value="중복된 닉네임입니다." />
                </c:when>
                <c:when test="${registerResult == RegisterResult.FAILURE}">
                    <c:set var="alert" value="알 수 없는 이유로 회원가입에 실패하였습니다" />
                </c:when>
            </c:choose>
        alert('${alert}');
            <c:choose>
...
<main>
    <h1>회원가입</h1>
    <form method="post" rel="register-form" accept-charset="UTF-8">
        <table>
            <tr>
                <th>
                ...
                <td>
                    <input type="text" name="id" id="id-input" placeholder="아이디" autofocus>
                    <span class="input-message" rel="id-message"></span>
                </td>
            </tr>
            <tr>
                <th>
                ...
                <td>
                    <input type="password" name="password" id="password-input" placeholder="비밀번호">
            ...
            <tr>
                ...
                <td>
                    <input type="password" name="passwordCheck" id="password-check-input" placeholder="비밀번호">
            ...
            <tr>
                ...
                <td>
                    <input type="text" name="name" id="name-input" placeholder="이름">
                </td>
            </tr>
            <tr>
                <th>
            ...
            <tr>
                ...
                <td class="addressTd">
                    <input readonly type="text" name="addressPostal">
                    <input type="button" name="addressButton" value="주소 찾기" id="address-input">
                    <br>
                    <input readonly type="text" name="addressPrimary" placeholder="기본 주소">
                    <br>
                    <input type="text" name="addressSecondary" placeholder="상세 주소">
                </td>
            </tr>
            <tr>
                <th>
                    <label for="email-input">이메일 인증</label>
                </th>
                <td>
                    <input id="email-input" type="email" name="email" placeholder="이메일">
                    <span class="input-message " rel="email-message">올바른 이메일을 입력하세요.</span>
                </td>
            </tr>
        </table>
        <table>
...
회사는 개인정보를 수집, 이용목적 이외에 다른 용도로 이용하거나 회원의 동의 없이 제3자에게 이를 제공하지 않습니다.
        ...
        <table>
            ...
            <tr>
                <td>
                <textarea readonly>원칙적으로, 개인정보 수집 및 이용목적이 달성된 후에는 해당 정보를 지체 없이 파기합니다.
단, 관계법령의 규정에 의하여 보존할 필요가 있는 경우 회사는 아래와 같이 관계법령에서 정한 일정한 기간 동안 회원정보를 보관합니다.

보존 항목 : 이름 , 생년월일 , 성별 , 로그인ID , 비밀번호 , 휴대전화번호 , 이메일 , 신용카드 정보 , 은행계좌 정보 , 서비스 이용기록 ,
접속 로그 , 쿠키 , 접속 IP 정보 , 결제기록 , 닉네임
            ...
            <tr>
                <td>
                    <label>
                        <input type="checkbox" name="agreePrivacy">
                        <label></label>
                        <span>[필수]개인정보 보유 및 이용기간에 동의합니다.</span>
                    </label>
                </td>
            </tr>
            ...
            <tr>
                <td>
                    <label>
                        <input type="checkbox" name="agreePrivacy">
                        <label></label>
                        <span>[필수]개인정보 제3자 제공동의에 동의합니다.</span>
                    </label>
                </td>
            </tr>
