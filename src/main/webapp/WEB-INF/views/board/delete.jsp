...
<head>
    ...
    <link rel="stylesheet" href="${pageContext.request.contextPath}/resources/stylesheets/common.css">
        <c:choose>
            <c:when test="${deleteVo.result == DeleteResult.FAILURE}">
                <script>
                    alert('존재하지 않는 게시글입니다.');
                    window.history.back();
                </script>
            </c:when>
            <c:when test="${deleteVo.result == DeleteResult.NOT_ALLOWED}">
                <script>
                    alert('게시글을 삭제 할 권한이 없습니다.');
                    window.history.back();
                </script>
            </c:when>
            <c:when test="${deleteVo.result == DeleteResult.SUCCESS}">
                <script>
                    alert('게시글을 삭제하였습니다.');
                    window.location.href='/board/list/${deleteVo.boardCode}';
                </script>
            </c:when>
            <c:otherwise>
                <script>
